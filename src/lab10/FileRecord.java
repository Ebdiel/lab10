...
public class FileRecord implements Comparable<FileRecord> {
	...
	private static int binHelper(FileRecord[] shelve, int from,int to, FileRecord target) {
		if(shelve.length==0){
			return -1;
		}
		else if(to<from){
		...
		if (shelve[mid].equals(target)){

		return mid;
		}
		else if(shelve[mid].compareTo(target)<0){
			return binHelper(shelve, mid+1, to, target);
		}
		else{
			return binHelper(shelve, from, mid-1,target );
		}
	}
	...
	public static int binarySearch(FileRecord[] shelve, FileRecord target) {
		...
		return binHelper(shelve, 0,(shelve.length-1), new FileRecord(target) {
			@SuppressWarnings("unused")
			public boolean equals(FileRecord otherRecord) {
				return this.getID() == otherRecord.getID();
			}
	...
	public static void selHelp(FileRecord[] shelve, int tail){
		if(tail>=shelve.length){return;}
		for(int i=tail+1; i<shelve.length; i++){
			if(shelve[tail].compareTo(shelve[i])>=0){
				swap(shelve,tail, i);
			}
		}
		selHelp(shelve,tail+1);
	}
}
